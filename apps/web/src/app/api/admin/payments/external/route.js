...
export async function GET(request) {
  ...
  let p = 1;
  if (status) {
    where.push(`ep.status = $${p++}`);
    params.push(status);
  }
  if (q) {
    where.push(
      `(CAST(ep.payment_intent_id AS text) ILIKE $${p++} OR COALESCE(ep.lemon_tx_id,'') ILIKE $${p++} OR COALESCE(ep.type,'') ILIKE $${p++})`,
    );
    params.push(`%${q}%`, `%${q}%`, `%${q}%`);
  }
  ...
  const rows = await sql(
    `SELECT ep.*,
     ...
     LEFT JOIN payment_intents pi ON pi.id = ep.payment_intent_id
