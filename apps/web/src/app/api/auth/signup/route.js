...
export async function POST(request) {
  ...
  try {
    ...
    if (!emailRegex.test(email)) {
      return Response.redirect(
        `/account/signup?error=${encodeURIComponent("Invalid email address")}`,
        302,
      );
    }
    ...
    if (!passwordValidation.valid) {
      ...
      return Response.redirect(
        `/account/signup?error=${encodeURIComponent(errorMessage)}`,
        302,
      );
    }
    ...
    // Get user role (new users default to 'customer')
    const roleRows = await sql`SELECT role FROM auth_users WHERE id = ${userId} LIMIT 1`;
    const userRole = roleRows?.[0]?.role || 'customer';
    
    // Redirect based on role (though new signups will always be customer)
    ...
    const html = tinyRedirectHtml(redirectPath, "Creating your account…");
