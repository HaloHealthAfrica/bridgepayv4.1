      ...
      if (method === "bridge_wallet") {
        ...
        try {
          await applyFees({
            ...
            currency: grp.currency || "KES",
            merchantId: null,
            // Provide both wallet ids so the catalog can decide who pays (customer vs merchant)
            // while still allowing collection when the payer is the group owner wallet.
            customerWalletId: payerWallet?.id || null,
            merchantWalletId: payerWallet?.id || null,
          });
        } catch (e) {
