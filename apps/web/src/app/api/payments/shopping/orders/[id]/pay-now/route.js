...
export async function POST(request, { params }) {
  try {
    ...
    const idemKey =
      request.headers.get("Idempotency-Key") || `shopping-paynow-${id}`;
    ...
    if (order.payment_mode !== "PAY_NOW") return bad(400, "not_pay_now_mode");
    ...
    console.error("/api/payments/shopping/orders/[id]/pay-now POST error", e);
